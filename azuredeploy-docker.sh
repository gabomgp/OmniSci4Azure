...
if [ ! -z "$NVIDIA_GPU" ]
then
    sudo -u $MAPD_USERNAME echo "docker run --runtime=nvidia -v $MAPD_STORAGE/omnisci-docker-storage:/omnisci-storage -p 6273-6280:6273-6280 omnisci/core-os-cuda" >> $MAPD_STORAGE/StartMapD.sh 
else
    sudo -u $MAPD_USERNAME echo "docker run -d -v $MAPD_STORAGE/omnisci-docker-storage:/omnisci-storage -p 6273-6280:6273-6280 omnisci/core-os-cpu" >> $MAPD_STORAGE/StartMapD.sh 
fi
